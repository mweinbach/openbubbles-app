...
class HandleActions {
  static Future<int> saveHandleAsync(dynamic data) async {
    ...
    final updatePoster = data['updatePoster'] as bool;
    final updateBlocked = data['updateBlocked'] as bool;
    ...
    return Database.runInTransaction(TxMode.write, () {
      ...
      if (!updatePoster) {
        handle.posterPath = existing?.posterPath ?? handle.posterPath;
      }
      if (!updateBlocked) {
        handle.blocked = existing?.blocked ?? handle.blocked;
      }
