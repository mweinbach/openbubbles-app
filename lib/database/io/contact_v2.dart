...
class ContactV2 {
  ...
  bool get isSharedSuggestion => !isNative && displayName.startsWith('Maybe: ');
  ...
  ContactV2 save() {
    ...
    Database.runInTransaction(TxMode.write, () {
      id = Database.contactsV2.put(this);
    });
    return this;
  }

  static ContactV2? findOne({String? nativeContactId, String? address}) {
    if (kIsWeb) return null;

    if (nativeContactId != null) {
      final query = Database.contactsV2.query(ContactV2_.nativeContactId.equals(nativeContactId)).build();
      query.limit = 1;
    ...
    if (address != null) {
      for (final contact in Database.contactsV2.getAll()) {
        if (contact.hasMatchingAddress(address)) return contact;
      }
    }
  ...
  static ContactV2? findSharedSuggestionByAddress(String address) {
    if (kIsWeb) return null;

    for (final contact in Database.contactsV2.getAll()) {
      if (contact.isSharedSuggestion && contact.hasMatchingAddress(address)) {
        return contact;
      }
    }
