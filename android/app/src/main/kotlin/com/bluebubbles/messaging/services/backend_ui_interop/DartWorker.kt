...
class DartWorker(context: Context, workerParams: WorkerParameters): ListenableWorker(context, workerParams) {
    ...
    companion object {
        ...
        /**
         * Deliver a method-channel call directly to the Dart side, spinning up a background
         * FlutterEngine if the app is not already alive. Used by services that receive events
         * while the app is in the background (e.g. APNService for "APNMsg"/"SMSMsg"). This is the
         * non-WorkManager path — callers invoke it from their own coroutine scope.
         */
        suspend fun callMethod(applicationContext: Context, method: String, arguments: Map<String, Any>) {
            ...
            try {
                val engineToUse: FlutterEngine = MainActivity.getEngine() ?: workerEngine ?: run {
                    ...
                    suspendCancellableCoroutine<Unit> { cont ->
                        MethodChannel(engineToUse.dartExecutor.binaryMessenger, Constants.methodChannel).invokeMethod(method, arguments, object : MethodChannel.Result {
                            override fun success(result: Any?) {
                                Log.d(Constants.logTag, "Method $method completed successfully")
                                if (cont.isActive) cont.resume(Unit)
                                closeEngineIfNeeded(applicationContext)
                            }

                            override fun error(errorCode: String, errorMessage: String?, errorDetails: Any?) {
                                Log.e(Constants.logTag, "Method $method failed!")
                                if (cont.isActive) cont.resume(Unit)
                                closeEngineIfNeeded(applicationContext)
                            }

                            override fun notImplemented() {
                                Log.e(Constants.logTag, "Method $method not implemented on Dart side")
                                if (cont.isActive) cont.resume(Unit)
                                closeEngineIfNeeded(applicationContext)
                            }
                        })
        ...
        private fun closeEngineIfNeeded(applicationContext: Context, selfId: UUID? = null) {
            ...
            Timer().schedule(5000) {
                ...
                runBlocking {
                    engineCleanup.withLock {
                        ...
                        // Exclude the caller's own worker ID (when provided) — WorkManager may still
                        // report it as RUNNING even after the Dart method callback has completed,
                        // which would cause the engine to never be destroyed.
                        val currentWork = WorkManager.getInstance(applicationContext).getWorkInfosByTag(Constants.dartWorkerTag).get().filter { element -> !element.state.isFinished && (selfId == null || element.id != selfId) }
                    ...
                    suspendCancellableCoroutine { cont ->
                        MethodChannel(engineToUse!!.dartExecutor.binaryMessenger, Constants.methodChannel).invokeMethod(method, gson.fromJson(data, TypeToken.getParameterized(HashMap::class.java, String::class.java, Any::class.java).type), object : MethodChannel.Result {
                            override fun success(result: Any?) {
                                ...
                                closeEngineIfNeeded(applicationContext, id)
                            }
